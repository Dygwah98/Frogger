...
img(new LineImage(0.0f, 0.5f)), objects() {
    ...
    static float h = Graphics::getInstance().get_line_height();
    ...
    if(index%5 != 0 or index == 10)
    for(int i = 0; i < 4; ++i) {
        
        objects.push_back(new GameObject(std::get<0>(obj_initializer.at(index)[i]), 30.0f, std::get<2>(obj_initializer.at(index)[i]) ));
